...
impl Subset {
    ...
    /// Creates a new Subset with color values set to the max of corresponding color values
    /// of self and other Subset
    pub fn parent_subset(self, rhs: &Subset) -> Self {
        Self {
    ...
    pub fn power<T>(&self) -> T
    where
        T: Mul<T, Output = T>,
        u8: Into<T>,
    {
        self.red.into() * self.blue.into() * self.green.into()
    }
}
...
fn main() -> Result<()> {
    ...
    for line in lines {
        ...
        power_sum += game
            .subsets
            .iter()
            .fold(Subset::ZERO, |acc, x| acc.parent_subset(x))
            .power::<u32>()
    }
    print!("{}", power_sum);
    Ok(())
